/* 
 * The script check for agent connection status at an interval. 
 * It check for agent that connected or diconnect from the cht server.
 * 
 */
...
// retrieve all the agent their connection state changed
function getAgentConnectionState() {
        ...
        function (response, status) {
            if (status == 200) {
                ...
                if (response_data.status == "ok") {
                    ...
                    if (typeof response_data.connected_agent != "undefined") {
                        self.postMessage({
                            event_type: _AGENT_CONNECTED,
                            list: response_data.connected_agent
                        });
                    }

                    if (typeof response_data.disconnected_agent != "undefined") {
                        self.postMessage({
                            event_type: _AGENT_DISCONNECTED,
                            list: response_data.disconnected_agent
                        });
                    }
                }
...
self.addEventListener("message", (e) => {
    ...
    if (typeof set_interval_handler == "undefined") {
        ...
        set_interval_handler = setInterval(() => {
            /*form = new FormData();
            form.append("request", "agent_connect_state");
            form.append("time_offset", last_check_time_offset);*/

            form = "request=agent_connect_state&";
