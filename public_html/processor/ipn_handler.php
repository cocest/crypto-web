...
try {
    ...
    if ($ipn_status >= 100 || $ipn_status == 2) { // check if transaction was successfully
        $query = 
            'SELECT A.userID, A.transaction, A.amountInUSD, A.committed, B.availableBalance 
             FROM user_transactions AS A LEFT JOIN user_account AS B ON A.userID = B.userID 
             WHERE A.transactionID = ?  LIMIT 1';
        ...
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($user_id, $transaction_type, $amount_in_usd, $transaction_committed, $available_balance);
            $stmt->fetch();
            $stmt->close();
            ...
            if ($transaction_committed == 0) {
                ...
                if ($transaction_type == 'deposit') {
                    ...
                    try {
                        ...
                        $stmt = $conn->prepare($query); // prepare statement
                        $stmt->bind_param('sis', $_POST['address'], $committed, $_POST['txn_id']);
                        $committed = 1;
                        $stmt->execute();
                        $stmt->close();
        ...
        $query = 'SELECT userID, transaction, amountInUSD FROM user_transactions WHERE transactionID = ?  LIMIT 1';
        $stmt = $conn->prepare($query); // prepare statement
        $stmt->bind_param('s', $_POST['txn_id']);
        $stmt->execute();
        $stmt->store_result(); // needed for num_rows
        ...
        // check if transaction exist
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($user_id, $transaction_type, $amount_in_usd);
            $stmt->fetch();
            $stmt->close();
